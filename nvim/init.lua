-- [[ ASH & AMBER EDITION: MULTI-LANG OPTIMIZED ]]
...
do
  ...
  vim.api.nvim_create_autocmd({ "BufEnter", "FocusGained", "InsertLeave", "CmdlineLeave", "WinEnter" }, {
    group = num_group, callback = function() if opt.nu:get() and vim.fn.mode() ~= "i" then opt.relativenumber = true end end,
  })
  vim.api.nvim_create_autocmd({ "BufLeave", "FocusLost", "InsertEnter", "CmdlineEnter", "WinLeave" }, {
    group = num_group, callback = function() if opt.nu:get() then opt.relativenumber = false end end,
  })

  vim.diagnostic.config({
    update_in_insert = false, severity_sort = true, virtual_text = true, virtual_lines = false,
...
require("lazy").setup({
  ...
  { "folke/which-key.nvim", opts = { spec = { { "<leader>s", group = "[S]earch" }, { "<leader>h", group = "Git [H]unk" } } } },
  { "lukas-reineke/indent-blankline.nvim", main = "ibl" },
  { "folke/flash.nvim", opts = { modes = { search = { enabled = true } } }, keys = { { "s", mode = { "n", "x", "o" }, function() require("flash").jump() end } } },
  ...
  { "folke/noice.nvim", opts = { presets = { command_palette = true, lsp_doc_border = true } } },
  { "NvChad/nvim-colorizer.lua", opts = { user_default_options = { tailwind = true, mode = "virtualtext" } } },
  { "folke/zen-mode.nvim", opts = { window = { backdrop = 0.95, width = 120 } }, keys = { { "<leader>z", "<cmd>ZenMode<cr>" } } },
  ...
  { "saghen/blink.cmp", version = "1.*", opts = { keymap = { preset = "super-tab" }, appearance = { nerd_font_variant = "mono" }, signature = { enabled = true } } },
...
require("ibl").setup({
  indent = { char = "│" }, scope = { enabled = true, show_start = false, show_end = false },
  exclude = { filetypes = { "alpha", "neo-tree", "TelescopePrompt" } },
})
...
require("bufferline").setup({ options = { mode = "buffers", themeable = true, separator_style = "thin", always_show_bufferline = true } })
...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(event)
    ...
    local b_map = function(keys, func) vim.keymap.set("n", keys, func, { buffer = event.buf }) end
    ...
    if vim.bo[event.buf].filetype == "cpp" or vim.bo[event.buf].filetype == "c" then b_map("<leader>ch", "<cmd>ClangdSwitchSourceHeader<cr>") end
  end,
})

-- Language Servers
local servers = {
  clangd = { cmd = { "clangd", "--background-index", "--clang-tidy", "--header-insertion=iwyu", "--fallback-style=llvm" } },
  zls = {},
...
require("mason").setup({})
-- Tools and Formatters for Mason to install (stylua is a formatter, handled separately)
...
require("nvim-treesitter.configs").setup({
  ensure_installed = { "bash", "c", "cpp", "go", "python", "zig", "html", "lua", "markdown", "vim", "javascript", "typescript", "json" },
  auto_install = true,
...
dap.configurations.cpp = { { name = "Launch Project", type = "gdb", request = "launch", program = function() return vim.fn.input("Executable: ", vim.fn.getcwd() .. "/", "file") end, cwd = "${workspaceFolder}" } }
